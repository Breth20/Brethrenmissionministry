            ...
            if (weekDay === 5 && !events[dateKey]) {
                ...
                cell.addEventListener("click", function () {
                    ...
                    document.getElementById("eventDetails").innerHTML = `
                        ...
                        <p>
                            Everyone is welcome to join our weekly fellowship
                            for worship, prayer, Bible study, and ministry updates.
                        </p>
...
function setupLanguageSwitcher(selectId) {
    ...
    if (swahiliPages.includes(currentPage)) {
        selector.value = "sw";
    } else {
        selector.value = "en";
    }

    // Wait until the user actually changes the selector
    selector.addEventListener("change", function () {

        const selectedLanguage = this.value;

        const isSwahiliPage = swahiliPages.includes(currentPage);

        // Already on the selected language
        if (
            (selectedLanguage === "en" && !isSwahiliPage) ||
            (selectedLanguage === "sw" && isSwahiliPage)
        ) {
            return;
        }
